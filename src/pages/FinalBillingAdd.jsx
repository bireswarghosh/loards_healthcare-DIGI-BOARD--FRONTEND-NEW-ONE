...
const [lessAdvData, setLessAdvData] = useState([])
 ...
 console.log("total diag chrgs; ",totalDiag)
  ...
  const fetchBedsById = async (bedId) => {
    try {
      ...
      const res = await axiosInstance.get(`/bedMaster/${bedId}`);
      // console.log("bed: ", res.data.data);
      setFetchedAdmBedDetail([res.data.data]);
      const bedAllUptoToday = bedChargesUptoToday({
        AdmitionId: admData?.AdmitionId,
        NoofDays: 1,
        MyDate: admData?.AdmitionDate,
        BedId: admData?.BedId,
        BedRate: res.data.data.TotalCh,
        CGST: 0,
        SGST: 0,
      });
      ...
      setFormData((prev) => ({
        ...
        details: {
          ...prev.details,
          finalbillbeddtl: [
            ...prev.details.finalbillbeddtl,
            ...bedAllUptoToday,
          ],
        },
      }));

      setBedChargesData(bedAllUptoToday);
    } catch (err) {
  ...
  // function to add bed charge from the adm date to today
  const bedChargesUptoToday = (firstElement) => {
    // today date (without time)
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // starting date
    let currentDate = new Date(firstElement.MyDate);
    ...
    while (currentDate <= today) {
      ...
      const newObj = {
        ...firstElement,
        MyDate: new Date(currentDate).toISOString(),
      };
    ...
    const totalBedRate = resultArray.reduce(
  ...
  function transformOtherCharges(
    ...
    return otherChargesByAdmId.map((item, index) => {
      ...
      return {
        ...
        PrintHead: matched?.billPrintHeadName || "",
