...
const SendSticker = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axiosInstance.post('/whatsapp/send-sticker', payload);
        ...
        <form onSubmit={handleSubmit}>
          <div className="row g-3">
            ...
            <div className="col-12">
              <label className="form-label">Sticker URL (Image/GIF) *</label>
              <input type="url" name="url" className="form-control" value={formData.url} onChange={handleChange} placeholder="https://example.com/image.jpg" required />
              <small className="text-muted">Direct URL only (not Google Drive or Dropbox)</small>
            </div>
            <div className="col-12">
              <button type="submit" className="btn btn-primary" disabled={loading}>
                {loading ? 'Sending...' : 'Send Sticker'}
              </button>
            </div>
          </div>
        </form>
