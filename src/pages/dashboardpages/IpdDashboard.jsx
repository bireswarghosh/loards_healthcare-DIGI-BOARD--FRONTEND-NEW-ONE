...
const IpdDashboard = () => {
  ...
  const getPatientCount = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get("/patient-count", {
        params: {
          startDate,
          endDate,
        },
   ...
   const statsConfig = [
     {
       ...
       value: response.totalPatients,
       gradient: "linear-gradient(135deg, #3aa0ff, #2a7de1)",
       footer: "**** **** **** 1234",
     },
     {
       title: "FROM OPD",
       value: response.totalOpd,
       gradient: "linear-gradient(135deg, #7b2ff7, #5f1edc)",
       footer: "**** **** **** 5678",
     },
     {
       title: "INSURED PATIENTS",
       value: response.insured,
       gradient: "linear-gradient(135deg, #ff4e50, #c0392b)",
       footer: "**** **** **** 8899",
     },
     {
       title: "TOTAL REVENUE",
       value: response.totalPatients,
       gradient: "linear-gradient(135deg, #00c9a7, #00b894)",
       prefix: "₹",
       footer: "**** **** **** 4321",
     },
   ];
      ...
      <div className="dashboard-bg">
        {loading ? (
          ...
          <>
            ...
            {/* 📊 TABLE + GRAPH */}
            <div
              className="row g-4 dashboard-section"
              style={{ minHeight: "220px" }}
            >
              <div className="col-lg-8">
                <div className="dashboard-card">
                  {/* <AdmissionsTable
                    data={response.patientList}
