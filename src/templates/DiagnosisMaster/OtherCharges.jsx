...
import PrintOtherCharges from "./PrintOtherCharges";
...
const OtherCharges = () => {
  ...
  useEffect(() => {
    ...
    if (shouldOpen === "true") {
      const RegId = sessionStorage.getItem("ID");
      openDrawerAdd();
      if (RegId) {
        setSelectedPatient({
          label: RegId,
        ...
        fetchPatientById(RegId); // ❗ SAME autofill logic re-used
      }
      sessionStorage.removeItem("openOtherChargesDrawer");
  ...
  //   }
  // };
  const loadSingle = async (id, rId) => {
    try {
      setLoading(true);
      const encodedId = encodeURIComponent(id);

      const response = await axiosInstance.get(
        `/opd-other-charges-with-items/${encodedId}/${encodeURIComponent(rId)}`
      );
  ...
  const openView = async (row) => {
    setModalType("view");
    await loadSingle(row.OutBillId, row.RegistrationId);
    setShowDrawer(true);
  };
  ...
  const handlePrint = () => {
    ...
    setTimeout(() => {
      ...
      <html>
        <head>
          <title>Bill-${formData.OutBillNo}</title>
          <style>
    ...
    `);
      ...
      printWindow.print();
      printWindow.close();
    }, 50); // delay ensures content is ready
  };
  ...
  const fetchPatientById = async (id) => {
    try {
      const res = await axiosInstance.get(
        `/patientregistration/${encodeURIComponent(id)}`
      );
                        ...
                        onChange={(e) => {
                          if (e.target.value === "Y") {
                            sessionStorage.setItem(
                              "openOtherChargesDrawer",
                              "true"
                            );
                            navigate("/UniversalPReg");
                          }
                        }}
                      >
                  ...
                  <div className="row g-2">
                    ...
                    <div className="col-md-3">
                      ...
                      <AsyncApiSelect
                        ...
                        labelKey="RegistrationId"
                        onChange={(opt) => {
                          if (!opt) return;
                          setSelectedPatient(opt);
                          fetchPatientById(opt.value);
                        }}
                      />
                    </div>
                  ...
                  <div className="d-flex gap-2 mt-3">
                    ...
                    {modalType !== "view" && (
                      <button
                        ...
                        onClick={
                          modalType === "add" ? handleAddSubmit : handleSubmit
                        }
                      >
                        Save
                      </button>
