...
const VisitTypeGrpMaster = () => {
  ...
  const fetchItems = async () => {
    setLoading(true)
    try {
      const response = await axiosInstance.get('/visit-type-groups')
      if (response.data.success) {
        setItems(response.data.data.map(item => ({ ...item, showDropdown: false })))
      }
    } catch (error) {
  ...
  const handleEdit = (item) => {
    setFormData({ VisitTypeGrp: item.VisitTypeGrp || '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalType === 'edit' && selectedItem) {
        const response = await axiosInstance.put(`/visit-type-groups/${selectedItem.VisitTypeGrpId}`, formData)
        if (response.data.success) {
          fetchItems()
          setShowModal(false)
        }
      } else {
        const response = await axiosInstance.post('/visit-type-groups', formData)
        if (response.data.success) {
          fetchItems()
          setShowModal(false)
        }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure?')) {
      try {
        setLoading(true)
        const response = await axiosInstance.delete(`/visit-type-groups/${id}`)
        if (response.data.success) {
          fetchItems()
        }
      } catch (error) {
  ...
  const filtered = items.filter(i => i.VisitTypeGrp?.toLowerCase().includes(searchTerm.toLowerCase()))
            ...
            <td>
              <div className="digi-dropdown dropdown d-inline-block" ref={dropdownRef}>
                ...
                <ul className={`digi-table-dropdown digi-dropdown-menu dropdown-menu dropdown-slim dropdown-menu-sm ${item.showDropdown ? 'show' : ''}`}>
                  ...
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleDelete(item.VisitTypeGrpId); }}><span className="dropdown-icon"><i className="fa-light fa-trash-can"></i></span> Delete</a></li>
                </ul>
              </div>
            </td>
            <td>{item.VisitTypeGrpId}</td>
            <td>{item.VisitTypeGrp}</td>
          </tr>
        ))}
      </tbody>
    </table>
        ...
        <>
          ...
          <div className={`profile-right-sidebar ${showModal ? 'active' : ''}`} style={{ zIndex: 9999, width: '100%', maxWidth: '500px', right: showModal ? '0' : '-100%', top: '70px', height: 'calc(100vh - 70px)' }}>
            ...
            <div className="top-panel" style={{ height: '100%', paddingTop: '10px' }}>
              <div className="dropdown-txt" style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#0a1735' }}>{modalType === 'add' ? '➕ Add' : modalType === 'edit' ? '✏️ Edit' : '👁️ View'} Visit Type Group</div>
              <OverlayScrollbarsComponent style={{ height: 'calc(100% - 70px)' }}>
                <div className="p-3">
                  <form onSubmit={handleSubmit}>
                    <div className="mb-3"><label className="form-label">📋 Visit Type Group *</label><input type="text" className="form-control" value={formData.VisitTypeGrp} onChange={(e) => setFormData({...formData, VisitTypeGrp: e.target.value})} disabled={modalType === 'view'} required /></div>
