...
const RemarksMaster = () => {
  ...
  const handleDropdownToggle = (event, index) => {
    ...
    setRemarks(remarks.map((data, i) => ({ ...data, showDropdown: i === index ? !data.showDropdown : false })));
  };

  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        setRemarks(remarks.map((data) => ({ ...data, showDropdown: false })));
      }
    };
  ...
  const fetchRemarks = async () => {
    setLoading(true)
    try {
      const response = await axiosInstance.get('/remarks')
      setRemarks((Array.isArray(response.data) ? response.data : []).map(item => ({ ...item, showDropdown: false })))
    } catch (error) {
  ...
  const handleEdit = (remark) => {
    setFormData({ Remark: remark.Remark || '' })
    setSelectedItem(remark)
    setModalType('edit')
    setShowModal(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalType === 'edit' && selectedItem) {
        await axiosInstance.put(`/remarks/${selectedItem.RemarkId}`, formData)
        fetchRemarks()
        setShowModal(false)
      } else {
        await axiosInstance.post('/remarks', formData)
        fetchRemarks()
        setShowModal(false)
      }
    } catch (error) {
  ...
  const filtered = remarks.filter(r => r.Remark?.toLowerCase().includes(searchTerm.toLowerCase()))
          ...
          <tr key={remark.RemarkId}>
            ...
            <td>
              <div className="digi-dropdown dropdown d-inline-block" ref={dropdownRef}>
                <button className={`btn btn-sm btn-outline-primary ${remark.showDropdown ? 'show' : ''}`} onClick={(e) => handleDropdownToggle(e, index)}>
                  Action <i className="fa-regular fa-angle-down"></i>
                </button>
                <ul className={`digi-table-dropdown digi-dropdown-menu dropdown-menu dropdown-slim dropdown-menu-sm ${remark.showDropdown ? 'show' : ''}`}>
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleView(remark); }}><span className="dropdown-icon"><i className="fa-light fa-eye"></i></span> View</a></li>
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleEdit(remark); }}><span className="dropdown-icon"><i className="fa-light fa-pen-to-square"></i></span> Edit</a></li>
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleDelete(remark.RemarkId); }}><span className="dropdown-icon"><i className="fa-light fa-trash-can"></i></span> Delete</a></li>
                </ul>
              </div>
            </td>
        ...
        <>
          ...
          <div className={`profile-right-sidebar ${showModal ? 'active' : ''}`} style={{ zIndex: 9999, width: '100%', maxWidth: '500px', right: showModal ? '0' : '-100%', top: '70px', height: 'calc(100vh - 70px)' }}>
            ...
            <div className="top-panel" style={{ height: '100%', paddingTop: '10px' }}>
              <div className="dropdown-txt" style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#0a1735' }}>{modalType === 'add' ? '➕ Add' : modalType === 'edit' ? '✏️ Edit' : '👁️ View'} Remark</div>
              <OverlayScrollbarsComponent style={{ height: 'calc(100% - 70px)' }}>
                <div className="p-3">
                  <form onSubmit={handleSubmit}>
                    <div className="mb-3"><label className="form-label">💬 Remark *</label><input type="text" className="form-control" value={formData.Remark} onChange={(e) => setFormData({...formData, Remark: e.target.value})} disabled={modalType === 'view'} required /></div>
