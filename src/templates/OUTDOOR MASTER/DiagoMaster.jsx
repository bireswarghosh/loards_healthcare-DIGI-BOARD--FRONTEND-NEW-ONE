...
const DiagoMaster = () => {
  ...
  const handleDropdownToggle = (event, index) => {
    ...
    const updatedData = diagos.map((data, i) => ({
      ...data,
      showDropdown: i === index ? !data.showDropdown : false,
    }));
    setDiagos(updatedData);
  };

  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        const updatedData = diagos.map((data) => ({ ...data, showDropdown: false }));
        setDiagos(updatedData);
      }
    };
  ...
  const fetchDiagos = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setDiagos(response.data.data.map(item => ({ ...item, showDropdown: false })))
      }
    } catch (error) {
      console.error("Error fetching diagos:", error)
      setError('Failed to fetch diagos')
    }
    setLoading(false)
  }
  ...
  const handleEdit = (diago) => {
    ...
    setSelectedItem(diago)
    setModalType('edit')
    setShowModal(true)
  }

  const handleView = (diago) => {
    setFormData({ diago: diago.diago || '' })
    setSelectedItem(diago)
    setModalType('view')
    setShowModal(true)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalType === 'edit' && selectedItem) {
        const response = await axiosInstance.put(`/diagos/${selectedItem.diagoId}`, formData)
        if (response.data.success) {
          fetchDiagos()
          setShowModal(false)
        }
      } else {
        const response = await axiosInstance.post('/diagos', formData)
        if (response.data.success) {
          fetchDiagos()
          setShowModal(false)
        }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this diago?')) {
      try {
        setLoading(true)
        const response = await axiosInstance.delete(`/diagos/${id}`)
        if (response.data.success) {
          fetchDiagos()
        }
      } catch (error) {
        console.error("Error deleting diago:", error)
        setError('Failed to delete diago')
      } finally {
        setLoading(false)
      }
    }
  ...
  const filteredDiagos = diagos.filter(diago => 
    diago.diago?.toLowerCase().includes(searchTerm.toLowerCase())
  )
  ...
  const renderTable = () => (
    <table className="table table-dashed table-hover digi-dataTable all-employee-table table-striped">
      ...
      <tbody>
        {filteredDiagos.map((diago, index) => (
          <tr key={diago.diagoId}>
            ...
            <td>
              <div className="digi-dropdown dropdown d-inline-block" ref={dropdownRef}>
                <button
                  className={`btn btn-sm btn-outline-primary ${diago.showDropdown ? 'show' : ''}`}
                  onClick={(event) => handleDropdownToggle(event, index)}
                >
                  Action <i className="fa-regular fa-angle-down"></i>
                </button>
                <ul className={`digi-table-dropdown digi-dropdown-menu dropdown-menu dropdown-slim dropdown-menu-sm ${diago.showDropdown ? 'show' : ''}`}>
                  <li>
                    <a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleView(diago); }}>
                      <span className="dropdown-icon"><i className="fa-light fa-eye"></i></span> View
                    </a>
                  </li>
                  <li>
                    <a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleEdit(diago); }}>
                      <span className="dropdown-icon"><i className="fa-light fa-pen-to-square"></i></span> Edit
                    </a>
                  </li>
                  <li>
                    <a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleDelete(diago.diagoId); }}>
          ...
          <div className={`profile-right-sidebar ${showModal ? 'active' : ''}`} style={{ zIndex: 9999, width: '100%', maxWidth: '500px', right: showModal ? '0' : '-100%', top: '70px', height: 'calc(100vh - 70px)' }}>
            ...
            <div className="top-panel" style={{ height: '100%', paddingTop: '10px' }}>
              <div className="dropdown-txt" style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#0a1735' }}>
                {modalType === 'add' ? '➕ Add' : modalType === 'edit' ? '✏️ Edit' : '👁️ View'} Diago
              </div>
