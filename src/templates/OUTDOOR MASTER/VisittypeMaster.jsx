...
const VisittypeMaster = () => {
  ...
  const [formData, setFormData] = useState({ VisitType: '' })
  ...
  const handleEdit = (item) => {
    setFormData({ VisitType: item.VisitType || '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalType === 'edit' && selectedItem) {
        await axiosInstance.put(`/visittypes/${selectedItem.VisitTypeId}`, formData)
        fetchItems()
        setShowModal(false)
      } else {
  ...
  const filtered = items.filter(i => i.VisitType?.toLowerCase().includes(searchTerm.toLowerCase()))
            ...
            <td>
              <div className="digi-dropdown dropdown d-inline-block" ref={dropdownRef}>
                ...
                <ul className={`digi-table-dropdown digi-dropdown-menu dropdown-menu dropdown-slim dropdown-menu-sm ${item.showDropdown ? 'show' : ''}`}>
                  ...
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleDelete(item.VisitTypeId); }}><span className="dropdown-icon"><i className="fa-light fa-trash-can"></i></span> Delete</a></li>
                </ul>
              </div>
            </td>
            <td>{item.VisitTypeId}</td>
        ...
        <>
          ...
          <div className={`profile-right-sidebar ${showModal ? 'active' : ''}`} style={{ zIndex: 9999, width: '100%', maxWidth: '500px', right: showModal ? '0' : '-100%', top: '70px', height: 'calc(100vh - 70px)' }}>
            ...
            <div className="top-panel" style={{ height: '100%', paddingTop: '10px' }}>
              <div className="dropdown-txt" style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#0a1735' }}>{modalType === 'add' ? '➕ Add' : modalType === 'edit' ? '✏️ Edit' : '👁️ View'} Visit Type</div>
              <OverlayScrollbarsComponent style={{ height: 'calc(100% - 70px)' }}>
                <div className="p-3">
                  <form onSubmit={handleSubmit}>
                    <div className="mb-3"><label className="form-label">📝 Visit Type *</label><input type="text" className="form-control" value={formData.VisitType} onChange={(e) => setFormData({...formData, VisitType: e.target.value})} disabled={modalType === 'view'} required /></div>
