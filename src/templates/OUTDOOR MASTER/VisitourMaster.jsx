...
const VisitourMaster = () => {
  ...
  const handleDropdownToggle = (event, index) => {
    ...
    setVisitours(visitours.map((data, i) => ({ ...data, showDropdown: i === index ? !data.showDropdown : false })));
  };

  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        setVisitours(visitours.map((data) => ({ ...data, showDropdown: false })));
      }
    };
  ...
  const fetchVisitours = async () => {
    ...
    try {
      ...
      setVisitours(response.data.map(item => ({ ...item, showDropdown: false })))
    } catch (error) {
      console.error("Error:", error)
      setError('Failed to fetch visitours')
    }
    setLoading(false)
  }
  ...
  const handleEdit = (visitour) => {
    ...
    setSelectedItem(visitour)
    setModalType('edit')
    setShowModal(true)
  }

  const handleView = (visitour) => {
    setFormData({ VisitOur: visitour.VisitOur || '' })
    setSelectedItem(visitour)
    setModalType('view')
    setShowModal(true)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (modalType === 'edit' && selectedItem) {
        await axiosInstance.put(`/visitours/${selectedItem.VisitOurId}`, formData)
        fetchVisitours()
        setShowModal(false)
      } else {
  ...
  useEffect(() => { fetchVisitours() }, [])

  const filtered = visitours.filter(v => v.VisitOur?.toLowerCase().includes(searchTerm.toLowerCase()))
          ...
          <tr key={visitour.VisitOurId}>
            ...
            <td>
              <div className="digi-dropdown dropdown d-inline-block" ref={dropdownRef}>
                <button className={`btn btn-sm btn-outline-primary ${visitour.showDropdown ? 'show' : ''}`} onClick={(e) => handleDropdownToggle(e, index)}>
                  Action <i className="fa-regular fa-angle-down"></i>
                </button>
                <ul className={`digi-table-dropdown digi-dropdown-menu dropdown-menu dropdown-slim dropdown-menu-sm ${visitour.showDropdown ? 'show' : ''}`}>
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleView(visitour); }}><span className="dropdown-icon"><i className="fa-light fa-eye"></i></span> View</a></li>
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleEdit(visitour); }}><span className="dropdown-icon"><i className="fa-light fa-pen-to-square"></i></span> Edit</a></li>
                  <li><a href="#" className="dropdown-item" onClick={(e) => { e.preventDefault(); handleDelete(visitour.VisitOurId); }}><span className="dropdown-icon"><i className="fa-light fa-trash-can"></i></span> Delete</a></li>
                </ul>
              </div>
            </td>
        ...
        <>
          ...
          <div className={`profile-right-sidebar ${showModal ? 'active' : ''}`} style={{ zIndex: 9999, width: '100%', maxWidth: '500px', right: showModal ? '0' : '-100%', top: '70px', height: 'calc(100vh - 70px)' }}>
            ...
            <div className="top-panel" style={{ height: '100%', paddingTop: '10px' }}>
              <div className="dropdown-txt" style={{ position: 'sticky', top: 0, zIndex: 10, backgroundColor: '#0a1735' }}>{modalType === 'add' ? '➕ Add' : modalType === 'edit' ? '✏️ Edit' : '👁️ View'} Visitour</div>
              <OverlayScrollbarsComponent style={{ height: 'calc(100% - 70px)' }}>
                <div className="p-3">
                  <form onSubmit={handleSubmit}>
                    <div className="mb-3"><label className="form-label">🕐 Visit Our *</label><input type="text" className="form-control" value={formData.VisitOur} onChange={(e) => setFormData({...formData, VisitOur: e.target.value})} disabled={modalType === 'view'} required /></div>
