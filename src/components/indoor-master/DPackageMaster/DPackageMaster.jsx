  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        const updatedData = dpackages.map((data) => ({
          ...data,
          showDropdown: false,
        }));
        setDPackages(updatedData);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (modalType === "edit" && selectedDPackage) {
        ...
        const response = await axiosInstance.post("/dpackage", formData);
  ...
  const fetchDPackages = useCallback(async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/dpackage?page=${currentPage}&search=${searchTerm}`
      );
      ...
      <tbody>
        {dpackages.map((dpackage, index) => (
          <tr key={dpackage.DpackageId}>
                ...
                >
                  ...
                  <li>
                    <a
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        handleDelete(dpackage.DpackageId);
                      }}
                    >
                      <span className="dropdown-icon">
            ...
            <td>
              <span className="badge bg-info">₹{dpackage.Rate}</span>
            </td>
