...
const ToolButton: React.FC<{
  ...
  <button
    onClick={onClick}
    title={title}
    className={`p-1.5 rounded hover:bg-[#DEEAF6] transition-colors ${active ? "bg-[#B4D6F5] shadow-inner" : ""} ${className}`}
  >
    {children}
  </button>
);
  ...
  <div className="grid grid-cols-10 gap-0.5 p-2">
    {colors.map((color) => (
      <button
        ...
        className={`w-5 h-5 rounded-sm border hover:scale-110 transition-transform ${currentColor === color ? "ring-2 ring-blue-500" : "border-gray-300"}`}
...
const RibbonHomeTab: React.FC<RibbonHomeTabProps> = ({ editor, formatPainterActive, onFormatPainterToggle }) => {
  ...
  const getCurrentFontSize = useCallback(() => {
    ...
    const attrs = editor.getAttributes("textStyle");
    if (attrs.fontSize) {
      return String(attrs.fontSize).replace("pt", "").replace("px", "").replace("rem", "");
    }
    return "11";
  }, [editor]);
  ...
  return (
    <div className="flex items-center gap-0.5 py-1 px-2 flex-wrap">
      <div className="flex items-center gap-0.5 pr-2 border-r border-gray-300 mr-2">
        <ToolButton onClick={() => void pasteFromClipboard(editor)} title="Paste (Ctrl+V)">
          <Clipboard size={16} />
        </ToolButton>
        <ToolButton onClick={() => void cutSelectionToClipboard(editor)} title="Cut (Ctrl+X)">
          <Scissors size={14} />
        </ToolButton>
        <ToolButton onClick={() => void copySelectionToClipboard(editor)} title="Copy (Ctrl+C)">
          <Copy size={14} />
        </ToolButton>
        <ToolButton
          onClick={onFormatPainterToggle}
      ...
      <div className="flex items-center gap-1 pr-2 border-r border-gray-300 mr-2">
        <select
          ...
          className="h-[28px] border border-gray-300 rounded text-xs px-1 w-[100px] bg-white focus:outline-none focus:border-blue-500"
          title="Styles"
        >
      ...
      <div className="flex items-center gap-1 pr-2 border-r border-gray-300 mr-2">
        <select
          ...
          className="h-[28px] border border-gray-300 rounded text-xs px-1 w-[130px] bg-white focus:outline-none focus:border-blue-500"
          title="Font Family"
        >
          {FONT_FAMILIES.map((f) => (
        ...
        <select
          ...
          className="h-[28px] border border-gray-300 rounded text-xs px-1 w-[52px] bg-white focus:outline-none focus:border-blue-500"
          title="Font Size"
        >
          {FONT_SIZES.map((s) => (
      ...
      <div className="flex items-center gap-0.5 pr-2 border-r border-gray-300 mr-2">
        ...
        <div className="relative" ref={textColorRef}>
          <div className="flex items-center">
            <button
              onClick={() => editor.chain().focus().setColor(currentTextColor).run()}
              className="p-1.5 rounded-l hover:bg-[#DEEAF6] transition-colors"
              title="Font Color"
            >
            ...
            <button
              onClick={() => {
                setShowTextColor((value) => !value);
                setShowHighlightColor(false);
              }}
              className="px-0.5 py-1.5 rounded-r hover:bg-[#DEEAF6] transition-colors"
            >
              <ChevronDown size={10} />
            </button>
          </div>
        ...
        <div className="relative" ref={highlightRef}>
          <div className="flex items-center">
            <button
              onClick={() => editor.chain().focus().toggleHighlight({ color: currentHighlight }).run()}
              className="p-1.5 rounded-l hover:bg-[#DEEAF6] transition-colors"
            ...
            <button
              onClick={() => {
                setShowHighlightColor((value) => !value);
                setShowTextColor(false);
              }}
              className="px-0.5 py-1.5 rounded-r hover:bg-[#DEEAF6] transition-colors"
            >
      ...
      <div className="flex items-center gap-0.5 pr-2 border-r border-gray-300 mr-2">
        ...
        <div className="relative" ref={lineSpacingRef}>
          <button
            onClick={() => setShowLineSpacing((value) => !value)}
            className="p-1.5 rounded hover:bg-[#DEEAF6] transition-colors flex items-center gap-0.5"
            title="Line Spacing"
          >
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              ...
              <polyline points="4 8 4 4 4 4" />
              <polyline points="4 16 4 20 4 20" />
