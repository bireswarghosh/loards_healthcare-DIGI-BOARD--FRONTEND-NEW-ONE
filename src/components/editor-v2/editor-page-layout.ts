...
function getPageDimensions(size: PageSize, orientation: PageOrientation) {
  let widthMm = 210;
  let heightMm = 297;
...
function getOrCreateRootWrapper(editorEl: HTMLElement): HTMLElement {
  ...
  if (parent.classList.contains(CONTENT_CLASS)) {
    ...
    if (maybeRoot && maybeRoot.classList.contains(ROOT_CLASS)) {
      return maybeRoot as HTMLElement;
    }
  }
...
export function applyEditorPageLayout(
  settings: Partial<PageLayoutSettings> = {},
) {
  ...
  const finalSettings: PageLayoutSettings = {
    ...DEFAULT_PAGE_LAYOUT,
    ...settings,
  };

  const root = getOrCreateRootWrapper(editorEl);
  const page = root.querySelector(`.${PAGE_CLASS}`) as HTMLElement | null;
  const content = root.querySelector(`.${CONTENT_CLASS}`) as HTMLElement | null;

  if (!page || !content) return;

  const dims = getPageDimensions(
    finalSettings.size,
    finalSettings.orientation,
  );
