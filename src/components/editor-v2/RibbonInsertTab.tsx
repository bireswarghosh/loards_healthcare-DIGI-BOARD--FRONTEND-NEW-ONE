...
const ToolButton: React.FC<{
  ...
  <button
    ...
    title={title}
    disabled={disabled}
    className={`p-1.5 rounded hover:bg-[#DEEAF6] transition-colors flex flex-col items-center gap-0.5 ${active ? "bg-[#B4D6F5] shadow-inner" : ""} ${disabled ? "opacity-40 cursor-not-allowed" : ""} ${className}`}
  >
    {children}
  </button>
);
...
const RibbonInsertTab: React.FC<RibbonInsertTabProps> = ({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (specialCharsRef.current && !specialCharsRef.current.contains(target)) {
  ...
  if (activeTab === "insert") {
    return (
      <div className="flex items-center gap-1 py-1 px-2 flex-wrap">
        ...
        <ToolButton onClick={handleInsertLink} title="Insert Link">
          <Link2 size={18} />
        ...
        >
          ...
          <span className="text-[9px]">Unlink</span>
        </ToolButton>
        ...
        <div className="relative" ref={specialCharsRef}>
          <ToolButton
            onClick={() => setShowSpecialChars((value) => !value)}
              ...
              <div className="grid grid-cols-10 gap-1">
                {SPECIAL_CHARS.map((char) => (
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center border border-gray-200 rounded hover:bg-blue-50 text-sm"
                  >
                    {char}
                  </button>
                ))}
        ...
        <ToolButton onClick={handleInsertDateTime} title="Insert Date & Time">
          <Clock size={18} />
        ...
        {editor.isActive("table") && (
          <>
            ...
            >
              ...
              <span className="text-[9px] text-red-500">-Row</span>
            </ToolButton>
            <ToolButton
              onClick={() => editor.chain().focus().deleteColumn().run()}
              title="Delete Column"
            >
              <Columns size={16} className="text-red-500" />
              <span className="text-[9px] text-red-500">-Col</span>
            </ToolButton>
            <ToolButton
            ...
            >
              ...
              <span className="text-[9px] text-red-500">Del</span>
            </ToolButton>
            <ToolButton
            ...
            >
              ...
              <span className="text-[9px]">Merge</span>
            </ToolButton>
            <ToolButton
      ...
      <div className="flex items-center gap-1 py-1 px-2 flex-wrap">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col items-center">
            ...
            if (val === "A4") {
              applyEditorPageLayout({ size: "A4" });
            } else if (val === "Letter") {
              applyEditorPageLayout({ size: "Letter" });
            } else if (val === "Legal") {
              applyEditorPageLayout({ size: "Legal" });
            }
          }}
            >
          ...
          <div className="flex flex-col items-center">
            ...
            if (val === "1") {
              applyEditorPageLayout({ columns: 1 });
            } else if (val === "2") {
              applyEditorPageLayout({ columns: 2 });
            } else if (val === "3") {
              applyEditorPageLayout({ columns: 3 });
            }
          }}
            >
  ...
  if (activeTab === "view") {
    return (
      <div className="flex items-center gap-1 py-1 px-2 flex-wrap">
        <ToolButton onClick={() => onFindOpen("find")} title="Find (Ctrl+F)">
          <Search size={18} />
          <span className="text-[9px]">Find</span>
        </ToolButton>
        <ToolButton onClick={() => onFindOpen("replace")} title="Replace (Ctrl+H)">
          <Replace size={18} />
          <span className="text-[9px]">Replace</span>
        </ToolButton>
        <div className="w-px h-8 bg-gray-300 mx-1" />
        <ToolButton onClick={onToggleRuler} title="Toggle Ruler" active={showRuler}>
          <Ruler size={18} />
          <span className="text-[9px]">Ruler</span>
        </ToolButton>
        <ToolButton onClick={onToggleGridlines} title="Toggle Gridlines" active={showGridlines}>
          <Grid3X3 size={18} />
          <span className="text-[9px]">Grid</span>
        </ToolButton>
        <ToolButton onClick={onToggleStatusBar} title="Toggle Status Bar" active={showStatusBar}>
          <PanelBottom size={18} />
  ...
  if (activeTab === "file") {
    return (
      <div className="flex items-center gap-1 py-1 px-2 flex-wrap">
        <ToolButton
          onClick={() => {
            if (confirm("Create a new document? Unsaved changes will be replaced in the editor.")) {
        ...
        <ToolButton onClick={onSaveDraft} title="Save Draft to Browser">
          <Save size={18} />
          <span className="text-[9px]">Save</span>
        </ToolButton>

        <ToolButton onClick={onSubmit} title="Submit Document">
          <FileUp size={18} />
          <span className="text-[9px]">Submit</span>
        </ToolButton>
        ...
        <div className="relative" ref={exportMenuRef}>
          ...
          >
            <FileDown size={18} />
            <span className="text-[9px] flex items-center gap-0.5">Save As <ChevronDown size={8} /></span>
          </ToolButton>
          {showExportMenu && (
            <div className="absolute top-full left-0 z-50 bg-white border border-gray-300 shadow-lg rounded mt-1 py-1 min-w-[180px]">
              <button
                onClick={() => {
                  void exportDocx(editor.getHTML(), documentTitle);
                  setShowExportMenu(false);
                }}
              ...
              <button
                onClick={() => {
                  printDocument(undefined, documentTitle);
                  setShowExportMenu(false);
                }}
                className="w-full text-left px-3 py-2 text-xs hover:bg-blue-50 flex items-center gap-2"
              >
              ...
              <button
                onClick={() => {
                  exportHtml(editor.getHTML(), documentTitle);
                  setShowExportMenu(false);
                }}
                className="w-full text-left px-3 py-2 text-xs hover:bg-blue-50 flex items-center gap-2"
              >
              ...
              <button
                onClick={() => {
                  exportTxt(editor.getText(), documentTitle);
                  setShowExportMenu(false);
                }}
                className="w-full text-left px-3 py-2 text-xs hover:bg-blue-50 flex items-center gap-2"
              >
                <FileDown size={14} /> Plain Text (.txt)
              </button>
            </div>
          )}
        </div>
        ...
        <ToolButton onClick={() => printDocument(editor.getHTML(), documentTitle)} title="Print (Ctrl+P)">
          <Printer size={18} />
