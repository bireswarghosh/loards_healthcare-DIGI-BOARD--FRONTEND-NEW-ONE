...
const FindReplace: React.FC<FindReplaceProps> = ({ editor, isOpen, onClose, mode: initialMode }) => {
  ...
  useEffect(() => {
    ...
    const rerender = () => setContentVersion((value) => value + 1);
      ...
      if (node.isText) {
        ...
        if (relativePos >= 0 && relativePos <= nodeText.length) {
          editor.commands.setTextSelection({
            from: nodePos + relativePos,
            to: nodePos + relativePos + searchText.length,
          });
  ...
  useEffect(() => {
    if (!searchText.trim() || matchPositions.length === 0) {
    ...
    setCurrentMatchIndex((previous) => {
      ...
      return Math.min(previous, matchPositions.length - 1);
    });
  ...
  const replaceCurrent = useCallback(() => {
    ...
    if (!matches) {
      if (matchPositions.length > 0) {
        selectMatch(currentMatchIndex < 0 ? 0 : currentMatchIndex);
      }
      return;
    }
  ...
  const replaceAll = useCallback(() => {
    ...
    for (let i = ranges.length - 1; i >= 0; i -= 1) {
      ...
      editor.chain().focus().insertContentAt(range, replaceText).run();
    }
    ...
    <div className="absolute top-0 right-4 z-50 bg-white border border-gray-300 shadow-lg rounded-b-lg w-[380px] print:hidden">
      ...
      <div className="p-3 space-y-2">
        <div className="flex items-center gap-2">
          ...
          <button onClick={findPrev} className="hover:bg-gray-100 rounded p-1" title="Previous">
            <ChevronUp size={16} />
          </button>
          <button onClick={findNext} className="hover:bg-gray-100 rounded p-1" title="Next">
            <ChevronDown size={16} />
          </button>
        </div>
        ...
        {mode === "replace" && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-xs bg-gray-100 hover:bg-gray-200 rounded px-2 py-1.5 border"
              title="Replace"
            >
              <Replace size={14} />
            </button>
            <button
              onClick={replaceAll}
              className="text-xs bg-gray-100 hover:bg-gray-200 rounded px-2 py-1.5 border whitespace-nowrap"
              title="Replace All"
            >
              All
            </button>
