...
const EditorCore: React.FC<EditorCoreProps> = ({
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent) => {
      ...
      menu.className =
        "fixed z-[9999] bg-white border border-gray-300 shadow-lg rounded py-1 min-w-[180px] text-xs";
  ...
  return (
    <div
      className="bg-[#E8E8E8] flex-1 min-h-0 overflow-auto overscroll-contain print:bg-white print:p-0"
    ...
    >
      <div className="mx-auto flex min-h-full w-full justify-center px-4 py-6">
        <div
          className="shrink-0"
          style={{
            width: `${BASE_PAGE_WIDTH * pageScale}px`,
                ...
                <div className="flex-1 relative h-[16px] bg-[#F0F0F0] rounded overflow-hidden">
                  {Array.from({ length: 17 }, (_, i) => (
                    ...
                    >
                      ...
                      {i % 2 === 0 && (
                        <span className="text-[7px] text-gray-500 leading-none">{i / 2}</span>
                      )}
                    </div>
                  ))}
                  <div className="absolute top-0 left-[12%] w-0 h-full border-l border-blue-400" />
                  <div className="absolute top-0 right-[12%] w-0 h-full border-r border-blue-400" />
                </div>
              </div>
            )}
