...
const Ribbon = ({
  editor, documentTitle, onDocumentTitleChange, formatPainterActive, onFormatPainterToggle,
  onFindOpen, onSaveDraft, onSubmit,
  ...
  return (
    <div style={{ background: "#F3F3F3", borderBottom: "1px solid #d1d5db", flexShrink: 0 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", height: "32px", background: "#185ABD", padding: "0 12px", gap: "8px" }}>
        ...
        {onSubmit && (
          <button onClick={onSubmit} style={{ color: "#fff", fontSize: "11px", padding: "4px 12px", borderRadius: "4px", border: "none", background: "rgba(255,255,255,0.15)", cursor: "pointer" }}
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #d1d5db", minHeight: "44px" }}>
        ...
        {(activeTab === "insert" || activeTab === "layout" || activeTab === "view" || activeTab === "file") && (
          <RibbonInsertTab
            editor={editor} activeTab={activeTab} documentTitle={documentTitle} onDocumentTitleChange={onDocumentTitleChange}
            onFindOpen={onFindOpen} onSaveDraft={onSaveDraft} onSubmit={onSubmit}
            showRuler={showRuler} onToggleRuler={onToggleRuler}
            showGridlines={showGridlines} onToggleGridlines={onToggleGridlines}
            showStatusBar={showStatusBar} onToggleStatusBar={onToggleStatusBar}
          />
        )}
      </div>
    </div>
