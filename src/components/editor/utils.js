...
function getTextRuns(node) {
  ...
  if (node.nodeType === Node.TEXT_NODE) {
    ...
    runs.push(new TextRun({ text, bold, italics, underline, strike, superScript, subScript }));
    return runs;
  }
      ...
      const tableRows = rows.map((row) => {
        ...
        const docxCells = cells.map((cell) => {
          return new TableCell({
            children: [new Paragraph({ children: getTextRuns(cell) })],
            borders: {
...
export function exportHtml(html, title) {
  const fullHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${title}</title><style>body{font-family:Calibri,sans-serif;font-size:11pt;line-height:1.15;max-width:816px;margin:0 auto;padding:96px}table{border-collapse:collapse;width:100%}td,th{border:1px solid #000;padding:8px}</style></head><body>${html}</body></html>`;
